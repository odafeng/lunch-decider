...
- 「幫我選一家」旁可直接指定料理，或選「不指定」。這個選單與上方料理篩選同步；上方複選時顯示「依上方複選」，不指定只清除料理限制，保留星等、評論數、距離、預算與收藏等條件。即時模式會重新搜尋，等待新結果期間停用抽選。
- 台式、中式、港式為獨立選項。Google 使用 `taiwanese_restaurant` 搜尋台式，以 `cantonese_restaurant`／`dim_sum_restaurant` 搜尋港式（含粵菜與飲茶）；詳見 [Google 官方類型表](https://developers.google.com/maps/documentation/places/web-service/place-types)。同時帶有籠統中式標籤時，優先採用台式或港式的具體分類。OSM 依 `cuisine` 標籤分類。缺少料理標籤的店家仍可在不指定時出現，不根據店名猜測分類。
